        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 sm:gap-5">
          {MEMBERS.map((member, i) => (
            <motion.div
              ...
              className="border border-border rounded-lg p-4 sm:p-5 text-center micro-card bg-card"
            >
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-primary text-primary-foreground flex items-center justify-center text-xs sm:text-sm font-bold mx-auto mb-4">
                {member.number}
              </div>
              ...
              {member.ypo && (
                <span className="inline-block bg-primary text-primary-foreground text-[0.55rem] font-bold uppercase tracking-wider px-2 py-0.5 rounded-sm">
                  YPO Member
                </span>
              )}
            </motion.div>
